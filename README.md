# Academic review template

A small, self-contained scaffold for writing, building, and archiving peer
reviews of academic manuscripts -- including re-reviews of revised submissions
...
(README, LICENSE, and the boilerplate `.gitignore` are omitted above.) Each
...
parent folders, so a per-round copy is what makes a LaTeX build produce a PDF
...
make submit             # copy the review into round-N/submitted/
make new                # scaffold the next round for a re-review
make help               # all targets
```

These work from the repository root. You can also `cd round-N/` and run `make`
there: each round carries a small shim `Makefile` that forwards to the root
Makefile with `ROUND` set from the folder name, so `make`, `make submit`, etc.
act on that round. (`make new` from inside a round still creates the next one.)

The build always focuses on the **latest round**. Creating a round shifts that
focus automatically: after `make new` (which scaffolds the next round), a bare
`make` builds the new round. To act on an earlier round -- or any specific one
-- name it explicitly with `ROUND=`, which works backwards too:
...
- otherwise it copies the raw `review.md` as `review-round-N.md` -- so a plain
  Markdown review you never built is archived as-is.

`make submit` does not build on its own, so it works standalone for a Markdown
submission; run `make` first when you want the PDF archived. It overwrites the
existing copy, which is fine -- the folder is version-controlled, so you can
...
assistants. Its scope is deliberately narrow, and worth stating plainly because
using LLMs to write peer reviews is rightly frowned upon: the assistant is meant
to **maintain the structure of the repository and provide an initial scaffold**
